...
@desc      :   an annotated edition of BCEmbedding produced by netease youdao.
'''
import torch
        ...
        with torch.no_grad():# set as no grad, which won't update the weight of model.
            ...
            for sentence_id in tqdm(range(0, len(sentences), batch_size), desc='Extract embeddings', disable=not enable_tqdm):
                ...
                if isinstance(query_instruction,str) and len(query_instruction)>0:
                    sentence_batch = [query_instruction + sent for sent in sentences[sentence_id:min(sentence_id+batch_size, len(sentences))]]
                else:
                    sentence_batch = sentences[sentence_id:min(sentence_id+batch_size, len(sentences))]
                #build batch.
                #using min to avoid overflow.
